...
channels = {}
try:
    with open("./providers/telly/telly_token.json", 'r') as openfile:
...
def get_catchup(id, utc, utcend):
    ...
    try:
        req = requests.get("https://backoffice0-vip.tv.itself.cz/contentd/api/device/getContent?device_token=" + token + "&channel_id=" + id + "&start=" + utc + "&end=" + utcend + "&broadcast_offset=").json()
        if req["success"] == True:
            url = req["stream_uri"]
    except:
        url = "http://sledovanietv.sk/download/noAccess-cs.m3u8"
