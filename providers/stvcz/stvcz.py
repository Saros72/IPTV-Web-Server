...
def get_catchup(id, utc, utcend):
    ...
    if sessid != "":
        ...
        if req["status"] == 1:
            ...
            req = requests.get("https://sledovanitv.cz/api/event-timeshift?format=m3u8&quality=" + quality + "&capabilities=" + kd + adaptive + "&force=true&eventId=" + eventId + "&overrun=1&PHPSESSID=" + sessid, headers = headers).json()
            if req["status"] == 1:
                url = req["url"]
    return url
