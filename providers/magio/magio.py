...
def get_data():
    try:
        with open("./providers/magio/magio_token.json", 'r') as openfile:
...
def set_data(data):
    ...
    with open("./providers/magio/magio_token.json", "w") as outfile:
...
def get_stream(id):
    ...
    req = requests.post("https://skgo.magio.tv/v2/auth/tokens", json = params, headers = headers).json()
    if req["success"] == True:
        accesstoken = req["token"]["accessToken"]
        refreshtoken = req["token"]["refreshToken"]
        set_data({"accesstoken": accesstoken, "refreshtoken": refreshtoken})
        params={"service": "LIVE", "name": dev_name, "devtype": "OTT_STB", "id": int(id.split(".")[0]), "prof": "p5", "ecid": "", "drm": "verimatrix"}
        ...
        if req["success"] == True:
            ...
            url = "http://sledovanietv.sk/download/noAccess-sk.m3u8"
            return url
    else:
        url = "http://sledovanietv.sk/download/noAccess-sk.m3u8"
        return url
...
def get_channels():
    ...
    ch = {}
    try:
        req = requests.get("https://skgo.magio.tv/home/categories?language=sk", headers = headers).json()["categories"]
        categories = {}
        for cc in req:
            for c in cc["channels"]:
                categories[c["channelId"]] = cc["name"]
        req = requests.get("https://skgo.magio.tv/v2/television/channels", params = params, headers = headers).json()["items"]
        for c in req:
...
def get_catchup(id, utc, utcend):
    ...
    if req["success"] == True:
        ...
        refreshtoken = req["token"]["refreshToken"]
        set_data({"accesstoken": accesstoken, "refreshtoken": refreshtoken})
        params={"service": "ARCHIVE", "name": dev_name, "devtype": "OTT_STB", "id": int(id), "prof": "p5", "ecid": "", "drm": "verimatrix"}
        headers = {"authorization": "Bearer " + accesstoken, 'Origin': 'https://www.magiogo.sk', 'Pragma': 'no-cache', 'Referer': 'https://www.magiogo.sk/', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'cross-site', 'User-Agent': UA}
        ...
        req = requests.get("https://skgo.magio.tv/v2/television/epg?filter=channel.id==" + str(id) + "%20and%20startTime=ge=" + d_start + ".000Z%20and%20endTime=le=" + d_end + ".000Z&limit=10&offset=0&lang=SK", params = params, headers = headers).json()
        if req["success"] == True:
            scheduleId = str(req["items"][0]["programs"][0]["scheduleId"])
            params={"service": "ARCHIVE", "name": dev_name, "devtype": "OTT_STB", "id": int(scheduleId), "prof": "p5", "ecid": "", "drm": "verimatrix"}
            headers = {"authorization": "Bearer " + accesstoken, 'Origin': 'https://www.magiogo.sk', 'Pragma': 'no-cache', 'Referer': 'https://www.magiogo.sk/', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'cross-site', 'User-Agent': UA}
            req = requests.get("https://skgo.magio.tv/v2/television/stream-url", params = params, headers = headers).json()
            if req["success"] == True:
                url = req["url"]
